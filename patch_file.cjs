const fs = require('fs');
let lines = fs.readFileSync('src/pages/ManageFees.jsx', 'utf8').split('\n');
const start = lines.findIndex(l => l.includes('{currentStructure.heads.map((h, i) => {'));
const end = lines.findIndex((l, i) => i > start && l.includes('})}')) + 1;

const replacer = `                                                {currentStructure.heads.map((h, i) => {
                                                                    ...
                                                                    <input 
                                                                        ...
                                                                        onChange={e => {
                                                                            ...
                                                                            setSelectedFees(prev => ({
                                                                                ...prev, 
                                                                                [h.name]: { selected: isChecked, months: isChecked ? ((h.frequency === 'Monthly' && unpaidMonths.length > 0) ? [unpaidMonths[0]] : []) : [] }
                                                                            }));
                                                                        }}
...
lines.splice(start, end - start, replacer);
fs.writeFileSync('src/pages/ManageFees.jsx', lines.join('\n'));
console.log("Patched array map correctly!");
